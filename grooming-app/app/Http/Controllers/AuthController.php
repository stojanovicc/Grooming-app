...
class AuthController extends Controller
{
    ...
    public function deleteAccount(Request $request)
    {
        ...
        if ($request->confirmation != 'yes') {
            return redirect()->route('profile.show', ['id' => $userId])->with('message', 'Brisanje naloga je otkazano.');
        }

        Salon::where('user_id', $user->id)->delete();

        $user->delete();

        Auth::logout();

        return redirect('/')->with('message', 'Vaš nalog je trajno obrisan.');
    }
}